...
namespace DAO
{
    public class CustomerDAO : DataProvider
    {
        ...
        public DataTable FindItems(string item)
        {
            ...
            {
                DataTable refult = new DataTable();
                string sql = "select * from Customer where customerCode like '%" + item + "%' or customerName like '%" + item + "%'";
                refult = GetData(sql);
                return refult;
            }
            catch (SqlException ex)
        ...
        public int Insert(Customer objCus)
        {
            ...
            {
                int result = 0;
                string sql = "insert into customer(customerCode, customerName, Address, SDT) values('" + objCus.cusCode + "','" + objCus.cusName + "','" + objCus.Address + "','" + objCus.SDT + "')";
        ...
        public bool checkCode(string Makh)
        {
            ...
            {
                bool refult = true;
                string sql = "select customerCode from Customer where customerCode = '" + Makh + "'";
